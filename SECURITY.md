...
If you believe you've found a security issue in Syntha, report it privately first.

This policy does two things: it gives researchers a clear disclosure path, and it spells out the trust model maintainers use when triaging reports. Syntha is a simulation platform for autonomous AI agents; it is not designed as a shared multi-tenant boundary between adversarial users on one gateway.
...
Security work is shared across a number of Syntha maintainers. See the [maintainer list](CONTRIBUTING.md#maintainers).

## Report a Security Issue

Report vulnerabilities directly to the repository where the issue lives:

- **Core simulation engine and CLI** — [anvarknian/Syntha](https://github.com/anvarknian/Syntha)

For issues that don't fit a specific repo, or if you're unsure, contact **@anvarknian** and we'll route it.

For Syntha core issues, submit through a private [GitHub Security Advisory](https://github.com/anvarknian/Syntha/security/advisories/new). Do not open a public issue or PR that discloses an unpatched vulnerability, exploit path, secret, or security-sensitive proof of concept.
...
Syntha does not currently run a paid bug bounty program. Please still disclose responsibly so we can fix real issues quickly. The best way to help the project right now is to send high-signal reports and, when practical, focused PRs.
...
- The actual impact, including which Syntha trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses a Syntha security boundary.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated Syntha impact.
...
- Tested version details (Syntha version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through Syntha. Showing that Syntha can reach a native parser is not enough by itself.
- Demonstrated impact tied to Syntha's documented trust boundaries.
- For exposed-secret reports: proof the credential is Syntha-owned (or grants access to Syntha-operated infrastructure/services).
...
Syntha does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-syntha-scopes` headers are ignored for that shared-secret path
...
- Syntha can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Syntha, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Syntha's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of Syntha's trusted computing base for a gateway.
...
- Using Syntha in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped Syntha production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.syntha`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Syntha-owned and not granting access to Syntha-operated infrastructure/services) without demonstrated Syntha impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.
- SSRF reports limited to the operator-managed HTTP/WebSocket proxy-routing feature where the demonstrated mitigation is to enable/configure `proxy.enabled` with a filtering `proxy.proxyUrl`/`SYNTHA_PROXY_URL`, or where impact depends on a permissive/misconfigured operator proxy. This only covers normal process-local HTTP(S)/WebSocket egress (`fetch`, Node HTTP(S), and similar JavaScript clients); non-HTTP egress and other features are assessed separately.

### Deployment Assumptions

Syntha security guidance assumes:

- The host where Syntha runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.syntha` state/config (including `syntha.json`) is effectively a trusted operator.
...
Syntha's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Syntha separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Syntha can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Syntha process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

Syntha uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/syntha` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/syntha` (or `syntha-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Syntha-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Syntha temp helpers (`resolvePreferredSynthaTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-syntha-dir.ts`
...
Syntha's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `syntha gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Syntha keeps deployment flexibility by design and does not hard-forbid non-local setups.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__syntha__/canvas/`, `/__syntha__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Syntha requires **Node.js 22.19.0 or later** (LTS). Node 24 is the recommended default runtime for new installs. The minimum version includes important security patches:
...
docker run --read-only --cap-drop=ALL \
  -v syntha-data:/app/data \
  syntha/syntha:latest
```

## Security Scanning

Syntha uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

Syntha runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.
