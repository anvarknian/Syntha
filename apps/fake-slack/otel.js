...
function maybeStartOtel() {
  ...
  try {
    ...
    console.warn('OTel SDK packages missing; fake-slack running without instrumentation')
    return
  }
  ...
  try {
    ...
    console.warn('OTLP trace exporter missing; fake-slack running without trace export')
  }
  ...
  if (startResult && typeof startResult.then === 'function') {
    startResult.then(() => {
      console.log('OTel Node SDK started (fake-slack)')
    }).catch(err => {
      console.error('Failed to start fake-slack OTel SDK', err)
    })
  } else {
    console.log('OTel Node SDK started (fake-slack)')
  }
...
async function shutdownOtel() {
  ...
  try {
    ...
    console.error('Error shutting down fake-slack OTel SDK', err)
  }
}
