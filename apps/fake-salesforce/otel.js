...
function maybeStartOtel() {
  ...
  try {
    ...
    console.warn('OTel SDK packages missing; fake-salesforce running without instrumentation')
    return
  }
  ...
  try {
    ...
    console.warn('OTLP trace exporter missing; fake-salesforce running without trace export')
  }
  ...
  if (startResult && typeof startResult.then === 'function') {
    startResult.then(() => {
      console.log('OTel Node SDK started (fake-salesforce)')
    }).catch(err => {
      console.error('Failed to start fake-salesforce OTel SDK', err)
    })
  } else {
    console.log('OTel Node SDK started (fake-salesforce)')
  }
...
async function shutdownOtel() {
  ...
  try {
    ...
    console.error('Error shutting down fake-salesforce OTel SDK', err)
  }
}
